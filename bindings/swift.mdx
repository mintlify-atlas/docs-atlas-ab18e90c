---
title: "Using livesplit-core from Swift"
sidebarTitle: "Swift"
...
// Timer_new returns a nullable pointer → factory method returns Timer?
guard let timer = Timer.new(run) else {
...
- `TimerRef` and `TimerRefMut` are non-owning references — never call `dispose()` on them
  directly.
- After passing an owned object to a consuming function (e.g. `Timer.new(run)`), the binding
